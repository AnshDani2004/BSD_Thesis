...
before any agent acts.  The CRN variance-reduction technique \citep{lecuyer1994efficiency}
...
= F_t + C_t(1 + m r_{\min})$.
This is $\geq F_t$ iff $1 + m r_{\min} \geq 0$, i.e., $|r_{\min}| \leq 1/m$.
In continuous time or for bounded distributions, this condition is always
satisfiable. In discrete time with heavy-tailed returns (e.g., Student-$t_3$),
a single shock $r_t < -1/m$ can breach the floor; thus, for tail-risk
environments, the CPPI guarantee remains probabilistic rather than absolute.
\end{proof}

\begin{remark}[Cost of Survival]
Proposition~\ref{prop:cppi_bound} \citep{black1992theory, busseti2016risk} makes precise the trade-off between safety
...
\begin{itemize}
    ...
    where $\hat\sigma_\downarrow^2 = T^{-1}\sum_t\min(R_t,0)^2$ is the
    \textbf{downside deviation} (the root mean square of negative returns).
    Unlike the Sharpe ratio, this penalizes only the "painful" volatility.
\end{itemize}
