...
# =============================================================================
# UCBAgent: Upper Confidence Bound (Exploration vs Exploitation)
# =============================================================================
...
class UCBAgent(BaseAgent):
    ...
    This agent balances exploration and exploitation by computing an optimistic
    estimate (upper confidence bound) of each arm's win probability, then
    using that optimistic estimate in the Kelly formula.
    
    Mathematical Basis: UCB1 algorithm (Auer et al., 2002)
    
    Expected Behavior: Explores arms more systematically than Thompson Sampling,
    eventually exploiting the best arm. Evaluates the explicit exploration vs.
    ...
    def __init__(
        ...
        Args:
            n_arms: Number of arms K
            odds: Payoff odds
            exploration_constant: Controls degree of exploration
            fraction_cap: Maximum total bet (safety cap)
        """
        super().__init__(n_arms, odds)
    ...
    def act(self, observation: Optional[np.ndarray] = None) -> np.ndarray:
        """
        Compute UCB for probability, then bet Kelly.
        """
        # If any arm has 0 pulls, play it (represented by setting p_ucb to 1.0)
        unplayed = self.pulls == 0
        if np.any(unplayed):
            p_ucb = np.where(unplayed, 1.0, 0.0)
        else:
            p_hat = self.wins / self.pulls
            exploration_term = np.sqrt(
                (self.exploration_constant * np.log(self.step_count)) / self.pulls
            )
            p_ucb = p_hat + exploration_term
            p_ucb = np.clip(p_ucb, 0.0, 1.0)
            
        kelly = compute_kelly_fraction(p_ucb, self.odds)
        return normalize_bets(kelly, self.fraction_cap)
        
    def update(self, outcomes: np.ndarray):
        """Update pull counts and win counts."""
        self.pulls += 1
        wins_this_step = (outcomes > 0).astype(np.float64)
        self.wins += wins_this_step
        self.step_count += 1
        
    def reset(self):
        """Reset internal memory."""
        self.pulls = np.zeros(self.n_arms)
        self.wins = np.zeros(self.n_arms)
        self.step_count = 0
        
    def get_state(self) -> dict:
        state = super().get_state()
        state['pulls'] = self.pulls.tolist()
        state['wins'] = self.wins.tolist()
...
# =============================================================================
# Epsilon-Greedy Kelly Agent
# =============================================================================

class EpsilonGreedyKelly(BaseAgent):
    """
    Classic Epsilon-Greedy strategy adapted for Kelly betting.
    
    With probability epsilon, explores by betting uniformly on all arms.
    With probability 1-epsilon, exploits the best known arm based on
    ...
    def act(self, observation: Optional[np.ndarray] = None) -> np.ndarray:
        p_hat = np.where(self.pulls == 0, 1.0, self.wins / np.maximum(self.pulls, 1))
        
        if np.random.random() < self.epsilon:
            # Explore: uniform positive expectation
            p_target = np.ones(self.n_arms) * np.max(p_hat)
        else:
            # Exploit: focus only on the best arm
            best_arm = np.argmax(p_hat)
            p_target = np.zeros(self.n_arms)
            p_target[best_arm] = p_hat[best_arm]
            
        kelly = compute_kelly_fraction(p_target, self.odds)
        return normalize_bets(kelly, self.fraction_cap)
...
# =============================================================================
# EXP3 Agent (Adversarial Bandits)
# =============================================================================
...
class EXP3Agent(BaseAgent):
    ...
    Mathematically proven to bound regret in nonstationary and adversarial
    environments. Maintains probability weights over arms and samples from them,
    updating weights exponentially based on received rewards.
    """
    ...
    def __init__(
        ...
        self.last_action_probs = self.probs.copy()
        
    def act(self, observation: Optional[np.ndarray] = None) -> np.ndarray:
        # Calculate sampling probabilities
        self.probs = (1.0 - self.gamma) * (self.weights / np.sum(self.weights)) + (self.gamma / self.n_arms)
        self.last_action_probs = self.probs.copy()
        
        # In multi-armed setting, you sample ONE arm. Since we distribute resources,
        # we will use the EXP3 probabilities directly as pseudo-expectations for Kelly,
        # or simply bet fractions proportional to probs. To match Kelly sizing, we'll
        # scale Kelly limits by these adversarial weights.
        # Approximation for robust fractional betting:
        kelly_base = 0.5 * self.fraction_cap # flat base 
        fractions = self.probs * kelly_base
        return np.clip(fractions, 0.0, self.fraction_cap)
        
    def update(self, outcomes: np.ndarray):
        # EXP3 receives scaled rewards.
        # outcome is usually return in [-1, b]. Map to [0, 1] pseudo-reward for EXP3.
        # Typical max return is odds. 
        max_return = np.max(self.odds)
        pseudo_rewards = (outcomes + 1.0) / (max_return + 1.0) 
        pseudo_rewards = np.clip(pseudo_rewards, 0.0, 1.0)
        
        # Update weights based on estimated rewards
        estimated_rewards = pseudo_rewards / self.probs
        self.weights *= np.exp((self.gamma * estimated_rewards) / self.n_arms)
...
class SoftmaxKellyAgent(BaseAgent):
    """
    Boltzmann Exploration adapted for Kelly betting.
    
    Probabilities are assigned smoothly based on the empirical estimates and 
    a temperature parameter tau.
    """
    ...
    def act(self, observation: Optional[np.ndarray] = None) -> np.ndarray:
        p_hat = np.where(self.pulls == 0, 0.5, self.wins / np.maximum(self.pulls, 1))
        
        # Softmax over probabilities
        exp_p = np.exp(p_hat / self.tau)
        softmax_weights = exp_p / np.sum(exp_p)
        
        # Kelly of individual arms scaled by softmax assignment
        kelly = compute_kelly_fraction(p_hat, self.odds)
        fractions = softmax_weights * kelly
        
        return normalize_bets(fractions, self.fraction_cap)
