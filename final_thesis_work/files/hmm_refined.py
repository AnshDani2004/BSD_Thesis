...
This module implements a refined HMM that addresses the Sticky Prior Paradox:
1. 2D observations: [return, rolling_volatility]
2. Log-space forward algorithm with logsumexp (numerical stability)
3. CUSUM change-point detection for rapid regime switches
4. Constrained transition matrix (A_ii <= 0.95, prevents sticky-prior re-emergence)

Mathematical Basis:
- Emission: b_j(y_t) = N(r_t; mu_j, sigma_j) x Gamma(v_t; k_j, theta_j)
- Forward: ln alpha_t(j) = ln b_j(y_t) + logsumexp_i(ln alpha_{t-1}(i) + ln A_ij)
- CUSUM: S_t^- = max(0, S_{t-1}^- - z_t - k_drift)

IMPORTANT -- Specified (not learned) emission parameters:
  This is a Bayesian filter with a specified generative model, NOT a fully
  learned HMM. Emission parameters (mu_j, sigma_j, vol_shape, vol_scale) are
  set a priori based on domain knowledge of bull/bear market return
  distributions rather than estimated via the Baum-Welch EM algorithm.
  Rationale: with T~250 observations, EM can overfit the emission model;
  a priori specification yields a more robust filter and avoids identifiability
  issues with short time series.

References:
- Hamilton (1989): Regime-switching models in macroeconomics
- Page (1954): CUSUM change-point detection
- Busseti, Ryu, Boyd (2016): Risk-constrained Kelly optimization
"""
